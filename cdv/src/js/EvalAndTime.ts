...
export class EvalAndTime {
  ...
  constructor(selector: any, options: any, data: any) {
    ...
    //eval guide lines
    let evalGuides = svg.append('g').attr('class', 'eval-guides');
    let evalGuideLines = [0.5, -0.5, 1, -1, 2, -2];
    ...
    let interactiveLayer = svg.append('g').attr('class', 'interactive-layer');
  ...
  update() {
    ...
    let y2max = d3.max(d3.extent(this._data, (d) => d.time).map(Math.abs));
    ...
    axes.select('g.axis.x').call(this._xAxis);
    axes.select('g.axis.yTime').call(this._yTimeAxis);
    axes.select('g.axis.yEval').call(this._yEvalAxis);
    ...
    //lines
    let lines = this.container.select('.lines').selectAll('.line');
    //enter
    lines
      .data(['white', 'black'])
      .enter()
    ...
    //update + enter
    lines.datum(this._data).transition().attr('d', line);

    //areas
    let areas = this.container.select('.areas').selectAll('.area');
    //enter
    areas
      .data(['white', 'black'])
      .enter()
    ...
    //update + enter
    areas.datum(this._data).transition().attr('d', area);
  }
}
