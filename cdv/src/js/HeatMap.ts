...
export class HeatMap {
  ...
  update() {
    ...
    let extent = d3.extent(
      this._data,
      (d) => d[this._options.accessor.piece][this._options.accessor.color]
    );
    ...
    heatSquares
      ...
      .attr(
        'x',
        (d, i) =>
          (i % 8) * this._options.squareWidth + this._options.squareWidth / 2
      )
      .attr(
        'y',
        (d, i) =>
          Math.floor(i / 8) * this._options.squareWidth +
          this._options.squareWidth / 2
      )
      ...
      .on('mouseenter', (d, i) => {
        this.dispatch.mouseenter(
          d[this._options.accessor.piece][this._options.accessor.color]
        );
      })
      .on('mousemove', (d, i) => {
    ...
    function squareSize(d) {
      ...
      if (self._options.sizeScale) {
        size = self._scale.size(
          d[self._options.accessor.piece][self._options.accessor.color]
        );
      } else {
